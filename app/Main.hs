...
import Graphics.Vulkan.Core as Core
import Linear.V2
...
findAndCreateDevice surface inner = do
  ...
  case m of
    Just (pd, qf) -> do
      let ci = DeviceCreateInfo [QueueCreateInfo qf [0]] ["VK_LAYER_LUNARG_standard_validation"] ["VK_KHR_swapchain"]
      withDevice pd ci $ do
        d <- ask
        q <- Vk.getQueue qf 0
        cp <- Vk.createCommandPool $ CommandPoolCreateInfo CreateResetCommandBuffer qf
        inner (pd, qf, d, q, cp)
    Nothing -> error "No Device"
  where
    findDevice [] = return Nothing
    findDevice (d:ds) = do
      x <- findQueue 0 =<< Vk.queueFamilyProperties d
      case x of
      ...
      where
        ...
        findQueue i (q:qs) = do
          s <- Vk.queueFamilySupportsPresent d i surface
      ...
      liftIO $ do
        ...
        pipeline <- createGraphicsPipeline device $
          ...
          (PipelineColorBlendStateCreateInfo zeroBits False ClearOp
            [PipelineColorBlendAttachmentState False Zero Zero Add Zero Zero Add Core.all]
            $ V.replicate 0)
