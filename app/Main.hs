...
findAndCreateDevice :: MonadIO m => Surface -> InstanceM m (PhysicalDevice, Word, Device, Queue, CommandPool)
findAndCreateDevice surface = do
  inst <- ask
  liftIO $ do
...
run window = do
  ...
  withInstance ici $ do
    ...
    (physicalDevice, qf, device, queue, commandPool) <- findAndCreateDevice surface
    liftIO $ do
